  ...
  const sequelize = new Sequelize(getConfig('DB_TOKEN'), {
    ...
    define: {
      ...
      hooks: {
        ...
        afterBulkCreate: (
          records: Model[],
          options: BulkCreateOptions<AnyTable>
        ) => {
          if (!getConfig('DB_INSERT')) {
            log.db(`[${getModelName(options)}] Records inserted!`, records);
          } else {
            log.db(`[${getModelName(options)}] Records inserted`);
          }
        ...
        afterBulkUpdate: (options: UpdateOptions<AnyTable>) => {
          const opts: UpdateOptions<AnyTable[]> & { model: { name: string } } =
            options as UpdateOptions<AnyTable[]> & { model: { name: string } };
          if (!getConfig('DB_UPDATE')) {
      ...
      try {
        ...
        if (options) {
          if (
            ...
            options.excludedModels.includes(file.split('.')[0])
          ) {
            shouldLoad = false;
          }

          if (
            options.includedModels &&
            !options.includedModels.includes(file.split('.')[0])
          ) {
            shouldLoad = false;
          }
        }

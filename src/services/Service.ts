...
import { IControllerResponse, IProviderResponse, IValidationStatus, ResponseCode } from "../types/General";
import { ParamsList, ServiceConstructor } from "../types/Service";
import { getContext } from "../loaders/routes/context";


const extractObjectStructure = (keys: Joi.Description): { [key: string]: string | Joi.Description } => {
	...
	for (const key in keys) {
		if (keys.hasOwnProperty(key)) {
			...
			if (type === "object" && nestedKeys) {
				...
				result[key] = `${type} ${flags?.presence ? "[" + flags?.presence + "]" : ""}`; // type || flags?.presence || "";
			}
		}
	}
...
export class Service {
	...
	public validateSchema(validator: Joi.ObjectSchema): IValidationStatus {
		const { error }: ValidationResult = validator.validate(this, { abortEarly: false });
		if (error) {
	...
	public static validateStaticParam(
		...
		const { error }: ValidationResult = validator.validate(payload, { abortEarly: false });
		if (error) {
			// Obtener la estructura del objeto esperado
			const objectStructure = extractObjectStructure(validator.describe().keys);
			...
			return {
				valid: false,
				errors: forcedMessage ? [forcedMessage] : error.details.map((detail) => detail.message),
				template: Object.keys(objectStructure).length > 0 ? objectStructure : undefined,
			};
		}
		return {
			valid: true,
	...
	public static validateStaticParams(params: ParamsList[]): IValidationStatus {
		...
		params.forEach((i) => {
			const { error }: ValidationResult = i.validator.validate(i.value, { abortEarly: false });
			if (error) {
				const objectStructure = extractObjectStructure(i.validator.describe().keys);
				errors = errors.concat(
					i.forcedMessage
						? [i.forcedMessage]
						: error.details.map((detail) => detail.message.replace("value", i.name))
				);
